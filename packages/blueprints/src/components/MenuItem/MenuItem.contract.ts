...
export default defineContract({
  ...
  rules: [
    ...
    {
      hint: 'For image preview decoding of newly selected files, prefer URL.createObjectURL(file). Create a new Image(), assign src, and use onload (or await img.decode() when available) to mark local readiness → fire onFileAccepted. Use onerror (or decode() rejection) to fire onFileError. Revoke object URL after state transition. Fallback to FileReader only if object URLs are unavailable. When using `initialValue` (URL), set the thumbnail src directly; optionally attach onerror to fall back to the placeholder. Do not emit callbacks on mount.'
    },

    // ✅ Canonical callback message strings — fixed (no runtime overrides)
    {
      hint: 'The callback payload `message` strings are fixed to the canonical values in `hints.messages`. The component must not expose props to override them and must emit exactly those strings in callbacks.'
    },

    // ✅ State flags discipline — what toggles each
    {
      hint: 'Toggle data flags precisely: data-drag-over=true during dragenter/over (clear on dragleave/drop); data-uploading=true after accept filter passes until local readiness or failure; data-accepted=true when showing the accepted UI; data-invalid=true when error text prop is non-empty.'
    },

    // ✅ Generation directives (enforce repo rules)
    {
      hint: 'Do not import the styleMap at runtime; compose className with literal tokens using composeClasses, attach `data-slot` attributes per slot, pass `group-hover:*` classes to Button/TextLink when rendered inside the dropzone, and export only `FileUploadProps` from the component file.'
    }
  ] as const,
  ...
  hints: {
    ...
    // Canonical live-region strings (fixed; tests rely on these exact values)
    dragMessages: {
      over: 'Drag files over',
      dropNow: 'Drop now',
