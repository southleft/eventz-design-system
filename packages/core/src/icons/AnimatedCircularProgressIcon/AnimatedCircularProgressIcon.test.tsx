...
import { AnimatedCircularProgressIcon } from './AnimatedCircularProgressIcon';

describe('AnimatedCircularProgressIcon', () => {
  it('defaults to decorative and sets aria-hidden="true"', () => {
    render(<AnimatedCircularProgressIcon data-testid="icon" />);
    expect(screen.getByTestId('icon')).toHaveAttribute('aria-hidden', 'true');
  });

  it('renders non-decorative icons with title as role="img" and aria-labelledby', () => {
    render(
      <AnimatedCircularProgressIcon decorative={false} title="Dismiss" titleId="close-icon" />
    );
  ...
  it('applies the color prop to the fill attribute', () => {
    render(<AnimatedCircularProgressIcon data-testid="icon" color="#123456" />);
    expect(screen.getByTestId('icon')).toHaveAttribute('fill', '#123456');
  });

  it('merges className onto the svg element', () => {
    render(<AnimatedCircularProgressIcon data-testid="icon" className="custom-class" />);
  ...
  it('generates a title id when non-decorative with title and no titleId', () => {
    render(<AnimatedCircularProgressIcon decorative={false} title="Dismiss" />);
