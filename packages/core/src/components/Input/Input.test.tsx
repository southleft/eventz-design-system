...
describe('Input native attribute passthrough', () => {
  ...
  it('merges user-provided aria-describedby with internal ids', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'Email info' });
...
describe('Input info popover', () => {
  it('renders the info trigger button when info text is present', () => {
    ...
    expect(screen.getByRole('button', { name: 'Email info' })).toBeInTheDocument();
  });
  ...
  it('removes the info content id from aria-describedby when the popover closes', async () => {
    ...
    await waitFor(() => {
      expect(input.getAttribute('aria-describedby') ?? '').not.toContain(content.id);
    });
  });
  ...
  it('restores focus to the info trigger after closing with Escape', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'Email info' });
    await user.click(trigger);
    await screen.findByText('We will contact you at this email.');
    await user.keyboard('{Escape}');
    await waitFor(() => {
