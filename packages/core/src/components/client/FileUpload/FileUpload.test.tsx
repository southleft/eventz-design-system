// packages/core/src/components/client/FileUpload/FileUpload.test.tsx
// Generated by AI (Eventz agents). Do not edit directly.
// Summary: Verifies FileUpload label, hint, and info behaviors, aria-describedby updates, accept filtering, drag states, preview lifecycle, callbacks, and dropzone state tokens.
...
describe('FileUpload', () => {
  ...
  it('accepts files by mime wildcard (e.g., image/*)', async () => {
    ...
    await waitFor(() => {
      const root = container.firstElementChild as HTMLElement;
      if (root.getAttribute('data-accepted') !== 'true') {
  ...
  it('processes dropped files via dataTransfer and accepts valid images', async () => {
    const { container } = render(<FileUpload label="Profile photo" />);
    const dropzone = screen.getByRole('group', { name: 'Profile photo' });
  ...
  it('marks the upload as accepted after preview decoding succeeds', async () => {
    const { container } = render(<FileUpload label="Profile photo" />);
    const fileInput = container.querySelector('input[type="file"]') as HTMLInputElement;
    const imageFile = createMockFile('photo.png', 'image/png');
    fireEvent.change(fileInput, { target: { files: [imageFile] } });
  ...
  it('falls back to setTimeout when requestAnimationFrame is unavailable', () => {
    ...
    // Spy on the global setTimeout used by the fallback scheduler
  ...
  it('ignores subsequent image load events after settling', async () => {
  ...
  it('emits onFileChanging when replacing an accepted file', async () => {
  ...
  it('removes an accepted image and resets preview/state', async () => {
    const { container } = render(<FileUpload label="Profile photo" />);
    const fileInput = container.querySelector('input[type="file"]') as HTMLInputElement;
    const imageFile = createMockFile('photo.png', 'image/png');
  ...
  it('falls back to the placeholder image when the preview image errors after acceptance', async () => {
    const { container } = render(<FileUpload label="Profile photo" />);
    const fileInput = container.querySelector('input[type="file"]') as HTMLInputElement;
    const imageFile = new File(['file'], 'photo.png', { type: 'image/png' });
