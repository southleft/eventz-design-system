...
In the Default column, print the exact JS/TS literal when available, even when it's an object (e.g., { placeholder: 'Select a date range', endIcon: <ArrowDropDownIcon /> }); do not use prose descriptions like "placeholder + Icon". Keep all props in alphabetical order.}}
