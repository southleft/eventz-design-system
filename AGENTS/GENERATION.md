...
Placement rules:
- **Client runtime (`/client/.../<ComponentName>.tsx`)**: first line must be `use client`; insert the 3-line doc block **after** that line, then a blank line, then imports.
- **Server runtime / stories / tests**: put the 3-line doc block at the very top, then a blank line, then imports.
- If a file already contains this exact pattern, do not duplicate it.
